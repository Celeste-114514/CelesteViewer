...
发布新版本
   ...
   ├─ 3. 打安装包  installer\CelesteViewer.nsi → dist\CelesteViewer-Setup-<版本>.exe
...
| 输出位置 | `dist\CelesteViewer-Setup-{版本}.exe`（项目根下的 dist 目录） |
...
```
dotnet publish CelesteViewer.csproj -c Release -p:Platform=x64 -p:SelfContained=true -p:WindowsAppSDKSelfContained=true
```
...
但发布目录里**没有 `CelesteViewer.pri`**。装到机器上一点开就死在
...
**已修的修法**：`CelesteViewer.csproj` 末尾加了 `CelesteCopyPriToPublish` target，
publish 收尾时自动把 `$(OutDir)$(MSBuildProjectName).pri` 复制进 `$(PublishDir)`。
发布日志里会打印一行 `CelesteViewer: PRI 资源索引已补入发布目录 → ...`；
...
`installer\CelesteViewer.nsi` 里的 `PUBLISH_DIR` 必须与之保持一致。
